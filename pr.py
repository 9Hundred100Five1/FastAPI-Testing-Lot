...
async def on_message(message):
    ...
    if message.content.startswith('깽구야 급식정보 '):
        try:
            ...
            embed=discord.Embed(color=0xff00, title="깽구가 불러온 급식정보", description="Info", timestamp=message.created_at)
    ...
    if message.content.startswith('깽구야 번역 '):
        ...
        embed=discord.Embed(color=0xff00, title="Translation", description="Info", timestamp=message.created_at)
        embed.set_footer(text=message.author, icon_url=message.author.avatar_url)
    ...
    if message.content.startswith("깽구야 공지 "):
        if message.author.id in owner:
            if str(message.content[7:]) == '' or str(message.content[7:]) == ' ':
                await message.channel.send("메세지를 작성하세요.")
            try:
                msg = message.content[7:]
                oksv = 0
                embed = discord.Embed(
...
client.run('ODI0NjY1ODc4MDM2NzQyMTQ0.YFyr8g.6k7E1Ty7HUId4QP8ugeay-tIIYk')